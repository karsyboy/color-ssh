...
fn replace_config_existing_auth_settings_are_replaced() {
    ...
    updated.auth_settings.idle_timeout_seconds = 42;
    updated.auth_settings.session_timeout_seconds = 84;

    replace_config(&shared, updated);

    let auth_settings = shared.read().expect("read config").auth_settings.clone();
    assert_eq!(auth_settings.idle_timeout_seconds, 42);
    assert_eq!(auth_settings.session_timeout_seconds, 84);
}
