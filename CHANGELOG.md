...
- Release v0.5.0 ([#29](https://github.com/karsyboy/color-ssh/pull/29))
...
- Hot config reloading well csh is running
- debug logging
- SSH session logging
- SSH logging secret redaction
- Launch title
- Add profile support
- Add fish and Zsh shell completion scripts
- Added support for windows,  macOS (beta), and linux.

### 🐛 Bug Fixes

- Debug formating issue
- *(logging)* Detect ssh hostname from arguments regardless of position
- *(highlighter.rs)* Fixed index mapping boundary bug in highlighter
- *(log/ssh.rs)* Logic error in SSH logger character filter
- *(lmacros)* Fixed logger instance being created on every macro call
- *(highlighter)* Rremoved unused variable 'filtered_matches'
- *(process)* Fixed rules being cloned on every chunk
- *(log/ssh.rs)* Fixed issue with ssh log file being repeatedly opened
- Fixed all cargo clippy errors

### 🚜 Refactor

- Updated more code and made more structure changes
- *(logging)* Enhance observability with comprehensive logging and documentation

### 📚 Documentation

- Updated network template config file and default config template
- Added documentation for shell completions extensions
- *(README)* Updated main readme to match everything that is new with v0.5
- *(arg)* Adjusted help page command examples
